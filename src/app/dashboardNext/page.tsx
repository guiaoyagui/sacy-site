...
export default function Dashboard() { // <-- CORRIGIDO: Nome do componente
  ...
  // --- INÍCIO DA CORREÇÃO: Refs e Hooks para KaTeX ---
  const resultContainerRef = useRef<HTMLDivElement>(null);
  
  useEffect(() => {
    // Verifica se a função de renderização do KaTeX está disponível na janela global
    if (result && resultContainerRef.current && (window as any).renderMathInElement) {
      try {
        (window as any).renderMathInElement(resultContainerRef.current, {
          // Define os delimitadores que o KaTeX deve procurar
          delimiters: [
            {left: '$$', right: '$$', display: true}, // Blocos de fórmula
            {left: '$', right: '$', display: false},  // Fórmulas inline
            {left: '\\(', right: '\\)', display: false}, // Delimitadores LaTeX
            {left: '\\[', right: '\\]', display: true} // Delimitadores LaTeX
          ],
          throwOnError: false
        });
      } catch (error) {
  ...
  }, [result]); // Re-renderiza o KaTeX sempre que 'result' (o HTML) muda
  // --- FIM DA CORREÇÃO: Refs e Hooks para KaTeX ---
  ...
  const runSearch = async (currentQuery: string) => {
    ...
    try {
      ...
      setResult(rawHtml); // <-- Usar rawHtml sem cleanHtmlResult para manter o bloco <style> do backend
  ...
  const handleSimplify = async () => {
    ...
    try {
      ...
      if (error.message && error.message.includes("403")) {
        errorMsg =
          "<p><strong>Erro na API Gemini (403 - Proibido)</strong></p><p>A chave da API pode estar em falta ou ser inválida.</p><hr>";
      }
  ...
  return (
    <>
      {/* --- INÍCIO DA CORREÇÃO: Scripts KaTeX (Carregamento no final do body) --- */}
      {/* Este método evita problemas de hidratação do Next.js e garante que os scripts carregam */}
      <script 
          dangerouslySetInnerHTML={{
            __html: `
              ...
              // Carrega o JS principal e o auto-render
              ...
              katexScript.onload = function() {
                  ...
                  renderScript.onload = function() {
                      // Se o elemento de resultado for alterado, renderiza
                      // A função renderMathInElement será chamada no React useEffect
                  };
      ...
      <div className="absolute inset-0 flex flex-col md:flex-row w-full h-screen bg-[#0f0f10] text-white overflow-hidden">
        ...
        <aside className="md:w-1/3 w-full p-6 border-r border-gray-800 flex flex-col h-full justify-between overflow-y-auto">
          ...
          <form onSubmit={handleFormSubmit} className="flex flex-col gap-3">
            {/* --- CORRIGIDO: TEXTAREA FLEXÍVEL --- */}
            <textarea
              rows={2} // AUMENTADO PARA COMEÇAR COM DUAS LINHAS
              ...
              // max-h-60 (240px) para garantir que a caixa não cresce mais do que ~10 linhas
              className="w-full p-3 rounded-xl bg-[#1a1a1b] text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none min-h-[60px] max-h-60 overflow-y-auto"
            />
            {/* --- FIM DA CORREÇÃO TEXTAREA --- */}
        ...
        <main className="flex-1 p-8 overflow-y-auto h-full">
          <div className="bg-[#161618] p-6 rounded-2xl shadow-lg min-h-[200px] h-full">
            ...
            {!loading && result && (
              <div
                ref={resultContainerRef} // Adicionado o ref aqui
