...
// API Key para a API Gemini
// (Deixe como "" - o ambiente irá fornecê-la)
const apiKey = "AIzaSyDTuho3L8K7DX0LYsIcLKYlLbRF3EN5gW4"; // <-- CORREÇÃO: Revertido de process.env
...
export default function DashboardNext() {
  ...
  // Guarda as respostas selecionadas: { questionIndex: alternativeIndex }
  const [selectedAnswers, setSelectedAnswers] = useState<{ [key: number]: number }>({});
  // Guarda os resultados após a correção: { questionIndex: 'correct' | 'incorrect' }
  const [quizResults, setQuizResults] = useState<{ [key: number]: "correct" | "incorrect" }>({});
  // Guarda a pontuação final
  const [quizScore, setQuizScore] = useState<string | null>(null);

  // --- NOVO HELPER PARA LIMPAR HTML (MAIS ROBUSTO) ---
  /**
   * Remove estilos inline indesejados (fundo branco, padding) do HTML
   * vindo do backend.
   */
  ...
  const runSearch = async (currentQuery: string) => {
    ...
    setLoading(true);
    setResult(null);
    setQuizQuestions([]); // Limpa quiz antigo
    setExtrasError(null);
    // --- Limpa os estados do quiz anterior ---
    setSelectedAnswers({});
    setQuizResults({});
    setQuizScore(null);
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/api/processar-consulta",
        {
          method: "POST",
      ...
      // --- USA O HELPER DE LIMPEZA ---
      const rawHtml = data.resumo || "<p><em>Nenhum resultado encontrado.</em></p>";
      ...
      // --- USA O HELPER DE LIMPEZA NO ERRO ---
  ...
  const handleSuggestQuestions = async () => {
    ...
    try {
      ...
      const jsonResponse = await callGeminiAPI(
        ...
        systemPrompt,
        true,
        quizSchema // Passa o schema detalhado
      );
  ...
  /**
   * Atualiza o estado quando o utilizador seleciona uma resposta.
   */
  const handleAnswerSelect = (questionIndex: number, alternativeIndex: number) => {
    // Só permite alterar se o quiz não tiver sido submetido
  ...
  /**
   * Compara as respostas selecionadas com as corretas e mostra os resultados.
   */
  ...
  return (
    <div className="absolute inset-0 flex flex-col md:flex-row w-full min-h-screen bg-[#0f0f10] text-white">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="bg-[#161618] p-6 rounded-2xl shadow-lg min-h-[200px]">
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="mt-4 flex flex-col items-center gap-4">
                  ...
                  >
                    {quizScore ? "Quiz Corrigido!" : "Corrigir Quiz"}
                  </button>
